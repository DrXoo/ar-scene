...
export class UIPositionHelper {
    ...
    public static setPosition(pivot: Vector3,
        ...
        if (positionType == PositionType.ABSOLUTE) {
           ...
           // this.setRelativePosition(pivot, position, )
        }
    }

    private static setRelativePosition(pivot: Vector3,
        componentPosition: Vector3,
        targetPosition: Vector3,
        uiObjectSize: Vector2) {
            componentPosition.set(uiObjectSize.x / 2, 0, 0);
            pivot.set(-uiObjectSize.x /2 + targetPosition.x, targetPosition.y, 0);
    }
